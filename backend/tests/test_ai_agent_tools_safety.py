...
def test_s3_allowlist_blocks_disallowed_prefix(monkeypatch):
    ...
    from app.services.agent_tools import aws_s3
    ...
    monkeypatch.setattr(aws_s3.s3_assets, "delete_object", _boom)
...
def test_sqs_allowlist_blocks_unknown_queue(monkeypatch):
    from app.settings import settings
    from app.services.agent_tools import aws_sqs
...
def test_github_allowlist_blocks_unknown_repo(monkeypatch):
    from app.settings import settings
    from app.services.agent_tools import github_api
