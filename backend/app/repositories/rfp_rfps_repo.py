...
def _ddb_key() -> Any:
    # Avoid static boto3 imports (keeps typecheckers happy in minimal setups).
    ...
    *, 
    ...
    try:
        ...
        try:
            ...
            stage = compute_stage(rfp=result, proposals_for_rfp=proposals)
        except Exception:
            stage = None
    ...
    # Minimal mode: Drive folder/template automation pruned.
...
def list_rfps(page: int = 1, limit: int = 20, next_token: str | None = None) -> dict[str, Any]:
    ...
    if not token and p > 1:
        for _ in range(1, p):
            pg = t.query_page(
                index_name="GSI1",
                key_condition_expression=_Key("gsi1pk").eq(type_pk("RFP")),
...
def list_rfp_proposal_summaries(rfp_id: str) -> list[dict[str, Any]]:
    ...
    while True:
        pg = t.query_page(
            key_condition_expression=_Key("pk").eq(f"RFP#{rfp_id}")
            & _Key("sk").begins_with("PROPOSAL#"),
            scan_index_forward=False,
            limit=200,
            next_token=tok,
        )
