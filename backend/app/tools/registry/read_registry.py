...
from ..categories.aws.aws_cognito import admin_get_user as cognito_admin_get_user
from ..categories.aws.aws_cognito import describe_user_pool as cognito_describe_user_pool
from ..categories.aws.aws_cognito import list_users as cognito_list_users
from ..categories.aws.aws_ecs import describe_service as ecs_describe_service
from ..categories.aws.aws_ecs import describe_task_definition as ecs_describe_task_definition
from ..categories.aws.aws_ecs import list_tasks as ecs_list_tasks
from ..categories.aws.aws_ecs import metadata_introspect as ecs_metadata_introspect
from ..categories.aws.aws_logs import tail_log_group as logs_tail, discover_log_groups_for_ecs_service as logs_discover_for_ecs, list_available_log_groups as logs_list_available
from ..categories.aws.aws_logs_insights import search_logs as telemetry_search_logs
from ..categories.aws.aws_logs_insights import top_errors as telemetry_top_errors
from ..categories.aws.aws_s3 import head_object as s3_head_object
from ..categories.aws.aws_s3 import presign_put_object as s3_presign_put_object
from ..categories.aws.aws_secrets import describe_secret as secrets_describe_secret
from ..categories.aws.aws_secrets import get_secret_value as secrets_get_secret_value
from ..categories.google.google_drive import (
    ...
    update_google_doc as google_update_doc,
    update_file_metadata as google_update_metadata,
    delete_google_file as google_delete_file,
)
from ..categories.aws.aws_sqs import get_queue_attributes as sqs_get_queue_attributes
...
from ...infrastructure.github.github_api import get_pull as github_get_pull
from ...infrastructure.github.github_api import list_check_runs as github_list_check_runs
from ...infrastructure.github.github_api import list_pulls as github_list_pulls
from ...infrastructure.github.github_api import discover_github_config as github_discover_config
from ..categories.slack.slack_read import create_canvas as slack_create_canvas
from ..categories.slack.slack_read import get_thread as slack_get_thread
from ..categories.slack.slack_read import list_recent_messages as slack_list_recent_messages
...
def _secrets_get_value_tool(args: dict[str, Any]) -> dict[str, Any]:
    sid = str(args.get("secretId") or "").strip()
    parse_json = bool(args.get("parseJson", True))
    try:
        return secrets_get_secret_value(secret_id=sid, parse_json=parse_json)
    except Exception as e:
        return {"ok": False, "error": str(e) or "secrets_get_value_failed"}


def _google_read_doc_tool(args: dict[str, Any]) -> dict[str, Any]:
    url = str(args.get("url") or "").strip() or None
    doc_id = str(args.get("documentId") or "").strip() or None
    try:
        return google_read_doc(url=url, document_id=doc_id)
    except Exception as e:
        return {"ok": False, "error": str(e) or "google_read_doc_failed"}


def _google_list_drive_files_tool(args: dict[str, Any]) -> dict[str, Any]:
...
def _google_create_doc_tool(args: dict[str, Any]) -> dict[str, Any]:
    ...
    folder_id = str(args.get("folderId") or "").strip() or None
    try:
        return google_create_doc(title=title, content=content, folder_id=folder_id)
    except Exception as e:
        return {"ok": False, "error": str(e) or "google_create_doc_failed"}


def _google_create_folder_tool(args: dict[str, Any]) -> dict[str, Any]:
    name = str(args.get("name") or "").strip()
    parent_folder_id = str(args.get("parentFolderId") or "").strip() or None
    try:
        return google_create_folder(name=name, parent_folder_id=parent_folder_id)
    except Exception as e:
        return {"ok": False, "error": str(e) or "google_create_folder_failed"}


def _google_upload_file_tool(args: dict[str, Any]) -> dict[str, Any]:
    name = str(args.get("name") or "").strip()
    ...
    if isinstance(content, str):
        ...
        return {"ok": False, "error": "content must be string or bytes"}
    
    mime_type = str(args.get("mimeType") or "").strip() or None
    folder_id = str(args.get("folderId") or "").strip() or None
    try:
        return google_upload_file(name=name, content=content_bytes, mime_type=mime_type, folder_id=folder_id)
    except Exception as e:
        return {"ok": False, "error": str(e) or "google_upload_file_failed"}


def _google_update_doc_tool(args: dict[str, Any]) -> dict[str, Any]:
    document_id = str(args.get("documentId") or "").strip()
    content = str(args.get("content") or "").strip() or None
    title = str(args.get("title") or "").strip() or None
    try:
        return google_update_doc(document_id=document_id, content=content, title=title)
    except Exception as e:
        return {"ok": False, "error": str(e) or "google_update_doc_failed"}


def _google_update_metadata_tool(args: dict[str, Any]) -> dict[str, Any]:
    file_id = str(args.get("fileId") or "").strip()
    name = str(args.get("name") or "").strip() or None
    folder_id = str(args.get("folderId") or "").strip() or None
    move_to_folder_id = str(args.get("moveToFolderId") or "").strip() or None
    try:
        return google_update_metadata(file_id=file_id, name=name, folder_id=folder_id, move_to_folder_id=move_to_folder_id)
    except Exception as e:
        return {"ok": False, "error": str(e) or "google_update_metadata_failed"}


def _google_delete_file_tool(args: dict[str, Any]) -> dict[str, Any]:
    file_id = str(args.get("fileId") or "").strip()
    permanent = bool(args.get("permanent", False))
    try:
        return google_delete_file(file_id=file_id, permanent=permanent)
    except Exception as e:
        return {"ok": False, "error": str(e) or "google_delete_file_failed"}
...
try:
    from ...shared.introspection.introspection_tools import get_introspection_tools
    _introspection_tools = get_introspection_tools()
except ImportError:
    _introspection_tools = {}

READ_TOOLS: dict[str, tuple[dict[str, Any], ToolFn]] = {
    **_memory_tools,
    **_introspection_tools,  # Introspection tools (list, search, introspect capabilities)
    **EXTERNAL_CONTEXT_TOOLS,  # External context tools (news, weather, research, etc.)
    ...
    "secrets_get_value": (
        tool_def(
            "secrets_get_value",
            "Get secret value from AWS Secrets Manager. ECS task has permissions to access any secret. Google secrets (GOOGLE_API_KEY, GOOGLE_CREDENTIALS) are automatically allowed.",
            {
                "type": "object",
                "properties": {
                    "secretId": {
                        ...
                        "minLength": 1,
                        "maxLength": 400,
                        "description": "ARN or name of the secret (e.g., arn:aws:secretsmanager:us-east-1:211125621822:secret:GOOGLE_API_KEY-yPu460)",
                    },
                    "parseJson": {
                        "type": "boolean",
                        "description": "If true, attempt to parse SecretString as JSON (default: true)",
                    },
                },
                "required": ["secretId"],
                "additionalProperties": False,
        ...
        _secrets_get_value_tool,
    ),
    "google_read_doc": (
        tool_def(
            "google_read_doc",
            "Read content from a Google Doc. Extracts text content from Google Docs URLs or document IDs. Automatically uses Google credentials from Secrets Manager.",
            {
                "type": "object",
                "properties": {
                    "url": {
                        ...
                        "minLength": 1,
                        "maxLength": 2000,
                        "description": "Google Docs URL (e.g., https://docs.google.com/document/d/14-58bKRlJ3B4GOLfgGw8yk3QGXp5KUG5-VliJIGSlWQ/edit)",
                    },
                    "documentId": {
                        "type": "string",
                        "minLength": 1,
                        "maxLength": 200,
                        "description": "Direct document ID (alternative to URL)",
                    },
                },
                "required": [],
                "additionalProperties": False,
    ...
    "google_create_doc": (
        tool_def(
            "google_create_doc",
            "Create a new Google Doc. Optionally set initial content and folder location.",
            {
                "type": "object",
                "properties": {
                    "title": {
                    ...
                    "content": {
                        "type": "string",
                        "maxLength": 1000000,
                        "description": "Optional initial content (plain text)",
                    },
                    "folderId": {
                        "type": "string",
                        "minLength": 1,
    ...
    "google_create_folder": (
        ...
        _google_create_folder_tool,
    ),
    "google_upload_file": (
        tool_def(
            "google_upload_file",
            "Upload a file to Google Drive. Content can be text (string) or binary (base64 encoded).",
            {
                "type": "object",
                "properties": {
                    "name": {
                    ...
                    "mimeType": {
                        "type": "string",
                        "maxLength": 200,
                        "description": "MIME type (e.g., 'text/plain', 'application/pdf'). Auto-detected from filename if not provided.",
                    },
                    "folderId": {
                        "type": "string",
                        "minLength": 1,
        ...
        _google_upload_file_tool,
    ),
    "google_update_doc": (
        tool_def(
            "google_update_doc",
            "Update a Google Doc (content and/or title). Replaces entire document content if content is provided.",
            {
                "type": "object",
                "properties": {
                    "documentId": {
        ...
        _google_update_doc_tool,
    ),
    "google_update_metadata": (
        tool_def(
            "google_update_metadata",
            "Update file metadata (rename, move to different folder).",
            {
                "type": "object",
                "properties": {
                    "fileId": {
                    ...
                    "folderId": {
                        ...
                        "minLength": 1,
                        "maxLength": 200,
                        "description": "New parent folder ID (moves file)",
                    },
                    "moveToFolderId": {
                        "type": "string",
                        "minLength": 1,
                        "maxLength": 200,
                        "description": "Alias for folderId (moves file to new folder)",
                    },
                },
                "required": ["fileId"],
                "additionalProperties": False,
        ...
        _google_update_metadata_tool,
    ),
    "google_delete_file": (
        tool_def(
            "google_delete_file",
            "Delete a file or folder from Google Drive. Can move to trash (default) or permanently delete.",
            {
                "type": "object",
                "properties": {
                    "fileId": {
                    ...
                    "permanent": {
                        "type": "boolean",
                        "description": "If true, permanently delete (cannot be recovered). If false, move to trash (default).",
                    },
                },
                "required": ["fileId"],
                "additionalProperties": False,
