...
def ensure_opportunity_exists_for_rfp(
    *,
    ...
    Back-compat: we use opportunity_id == rfp_id so that Opportunity (PROFILE)
    can share the same pk as existing OpportunityState rows (different sk).
    """
...
def list_opportunities(*, limit: int = 50, next_token: str | None = None) -> dict[str, Any]:
    """
    List opportunities (new view) via GSI1.
    """
    pg = get_main_table().query_page(
        index_name="GSI1",
        key_condition_expression=Key("gsi1pk").eq(_type_pk("OPPORTUNITY")),
